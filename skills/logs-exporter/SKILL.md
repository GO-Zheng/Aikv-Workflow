...
| 场景 | 命令 |
|------|------|
| 查看最近所有日志 | `cd /root/code/wiqun/Aikv-Workflow && ./scripts/export_logs.sh --duration=5m` |
| 查看 aikv 服务日志 | `cd /root/code/wiqun/Aikv-Workflow && ./scripts/export_logs.sh --service=aikv --duration=30m` |
| 导出 CSV 给 AI 分析 | `cd /root/code/wiqun/Aikv-Workflow && ./scripts/export_logs.sh --service=aikv --duration=1h --format=csv` |
...
```bash
# 过滤包含 ERROR 字样的日志行
cd /root/code/wiqun/Aikv-Workflow && ./scripts/export_logs.sh --service=aikv-logs --level=error --duration=30m
```
