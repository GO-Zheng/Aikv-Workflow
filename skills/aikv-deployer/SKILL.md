...
1. **必须先询问用户选择模式**：
   - `single`: 单节点模式（docker）
   - `cluster`: 集群模式（2 主 4 从，每分片 1 主 2 从，docker）
...
### docker-run-cluster
启动 AiKv 集群（2 主 4 从 Docker 容器，默认会初始化并测试）
```bash
...
- Replica-1a: 127.0.0.1:6380（50052）
- Replica-1b: 127.0.0.1:6381（50053）
- Master-2: 127.0.0.1:6382（50054）
- Replica-2a: 127.0.0.1:6383（50055）
- Replica-2b: 127.0.0.1:6384（50056）
