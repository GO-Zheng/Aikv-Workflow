...
| 曲线 | 曲线说明 | AiKv 中对应情况 | 计算公式 | 公式说明 |
|------|----------|-----------------|----------|----------|
| 趋势图 | 各类型命令每秒调用次数随时间变化 | 按 `cmd` 标签区分 GET/SET/MGET 等命令 | `rate(redis_commands_total[1m])` | 按命令类型 1 分钟 rate |

| 实际场景 | 现象 | 说明 |
|------|------|------|
| GET 为主 | 大量读操作 | 正常缓存场景 |
| SET 为主 | 写入压力大 | 检查业务写入模式 |
| 某类命令突增 | 该类型请求异常增加 | 检查是否有批量任务或攻击 |

#### 命令类型占比 (饼图)

| 曲线 | 曲线说明 | AiKv 中对应情况 | 计算公式 | 公式说明 |
|------|----------|-----------------|----------|----------|
| 占比 | 当前各类型命令占比 | 按 `cmd` 标签分组 | `rate(redis_commands_total[1m])` | lastNotNull 聚合 |

| 实际场景 | 现象 | 说明 |
|------|------|------|
| SET/GET 比例异常 | 写入占比过高 | 检查业务写入模式 |
| 某类命令占比 > 80% | 单一命令主导 | 考虑批量操作优化 |
| 命令类型单一 | 只有少数几种命令 | 正常缓存使用场景 |
...
| 指标                             | 采集器            | 含义          |
| ------------------------------ | -------------- | ----------- |
| redis_connected_clients        | redis_exporter | 当前活跃的客户端连接数 |
| redis_commands_processed_total | redis_exporter | 累计处理的命令总数   |
...
|------|------|------|
| 正常 | 连接数稳定在合理范围 | 正常运营 |
| 连接数持续高 | 接近机器上限 | 考虑扩容或优化连接池 |
| 连接数突增/突降 | 异常波动 | 检查应用连接池配置 |
...
| 显示内容 | 累计处理的命令总数 |
| 采集器 | redis_exporter |
| 计算公式 | `redis_commands_processed_total` |
...
| 指标 | 采集器 | 含义 |
|------|--------|------|
| redis_commands_latencies_usec_bucket | redis_exporter | 命令延迟直方图，le bucket 上存储累计请求数 |
...
| 曲线 | 曲线说明 | AiKv 中对应情况 | 计算公式 | 公式说明 |
|------|----------|-----------------|----------|----------|
| P50 (蓝色) | 50% 请求延迟低于此值 | 中位数延迟 | `histogram_quantile(0.50, sum(rate(...)) by (le)) / 1e6` | 转换为毫秒 |
| P95 (橙色) | 95% 请求延迟低于此值 | 高百分位延迟 | `histogram_quantile(0.95, sum(rate(...)) by (le)) / 1e6` | 转换为毫秒 |
| P99 (绿色) | 99% 请求延迟低于此值 | 极高百分位延迟 | `histogram_quantile(0.99, sum(rate(...)) by (le)) / 1e6` | 转换为毫秒 |

| 实际场景 | 现象 | 说明 |
|------|------|------|
| 正常 | P99 接近 P50 | 延迟分布稳定，无慢请求 |
| P99 远高于 P50 | 存在少量慢请求 | 排查大 key、网络问题 |
| P95/P99 突然升高 | 系统出现异常 | 可能存在阻塞操作或资源不足 |
| 各分位数都高 | 系统整体负载高 | 考虑扩容或优化性能 |

#### 命令类型延迟分布

| 曲线 | 曲线说明 | AiKv 中对应情况 | 计算公式 | 公式说明 |
|------|----------|-----------------|----------|----------|
| 各命令 P50 | 按命令类型展示延迟 | GET/SET/HGET 等各命令的延迟 | `histogram_quantile(0.50, sum(rate(...)) by (le, cmd)) / 1e6` | 按 cmd 分组 |

| 实际场景 | 现象 | 说明 |
|------|------|------|
| GET 延迟高 | 读取操作慢 | 检查存储层读取性能 |
| SET 延迟高 | 写入操作慢 | 检查写入和持久化性能 |
| 某命令延迟突增 | 该命令类型出现性能问题 | 针对性排查该命令逻辑 |
